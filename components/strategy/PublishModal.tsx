...
export default function PublishModal({ reportId, metrics, onClose }: PublishModalProps) {
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(data.error || "Failed to publish strategy");
            }
            ...
            router.push(`/strategy/${data.slug}`);
        } catch (err: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-[#0A0D14] border border-white/10 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
                <div className="p-8 border-b border-white/5 flex justify-between items-center bg-gradient-to-r from-indigo-500/10 to-transparent">
                    <div>
                        <h2 className="text-xl font-black text-white tracking-tight">Create Strategy Profile</h2>
                        <p className="text-xs text-gray-500 font-medium">Share your performance with the world</p>
                    </div>
                ...
                <div className="p-8 space-y-6">
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Strategy Name *</label>
                        <input
                            type="text"
                            value={name}
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Philosophy / Description</label>
                        <textarea
                            value={description}
                            onChange={(e) => setDescription(e.target.value)}
                            placeholder="Briefly explain the logic, session or edge of this strategy..."
                            rows={3}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Market</label>
                            <select
                                value={market}
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Symbol</label>
                            <input
                                type="text"
                                value={symbol}
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Visibility Mode</label>
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-widest ${visibility === "public" ? "text-white" : "text-gray-500"}`}>Public Profile</span>
                            </button>
                            <button
                            ...
                            >
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-widest ${visibility === "private" ? "text-white" : "text-gray-500"}`}>Private (Only Me)</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="bg-indigo-600/5 border border-indigo-500/10 rounded-2xl p-4 flex items-start gap-3">
                        ...
                        <p className="text-[10px] text-gray-500 leading-relaxed font-medium">
                            Public strategies appear in the library and search results. Private strategies are only visible to you. Your raw trade data is always kept secure and never shared.
                        </p>
                    </div>
                </div>
