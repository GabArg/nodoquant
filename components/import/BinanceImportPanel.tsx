...
const MARKET_OPTIONS: {
...
}[] = [
    {
        id: "spot",
        label: "Spot",
        description: "Spot trading history",
    },
    {
        id: "futures",
        label: "Futures (USDT-M)",
        description: "Perpetual & delivery contracts",
    },
    {
        id: "both",
        label: "Both",
        description: "Merge Spot + Futures trades",
    },
];

export default function BinanceImportPanel({
    onComplete,
    onBack,
}: Props) {
    ...
    const handleFetch = async () => {
        ...
        try {
            ...
            const res = await fetchBinanceTrades({
                apiKey: apiKey.trim(),
            ...
            if (res.trades.length === 0) {
                setError(
                    "No trades found. Make sure your API key has Read access and you have closed positions."
                );
                setLoading(false);
                return;
            }
                ...
                <>
                    ...
                    >
                        ...
                        <div>
                            ...
                            They are sent over HTTPS, used once to fetch your
                            trade history, and immediately discarded. We
                            recommend creating a <strong>read-only</strong> API
                            key.
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <ol
                            className="text-xs mt-3 space-y-1.5 leading-relaxed"
                            style={{ color: "#9ca3af" }}
                        >
                            <li>
                                1. Log in to Binance → click your profile →{" "}
                            ...
                            <li>
                                ...
                                permissions — <em>do not</em> enable trading or
                                withdrawals
                            </li>
                            <li>
                                4. Copy the API Key and Secret Key, then paste
                                them above
                            </li>
                        </ol>
                    </details>
                    ...
                    {result && (
                        ...
                        >
                            ✓ Fetched{" "}
                            {result.spotCount + result.futuresCount} trades
                            {result.spotCount > 0 &&
                                result.futuresCount > 0 &&
                                ` (${result.spotCount} Spot + ${result.futuresCount} Futures)`}
                        </div>
                    )}
