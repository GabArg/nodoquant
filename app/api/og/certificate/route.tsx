...
export async function GET(req: Request) {
    try {
        ...
        const metrics = isRecord(report.metrics_json) ? report.metrics_json : {};
        const strategy = (report.strategies as StrategySummary[] | null)?.[0] ?? null;
        ...
        const wr = rawWinRate * (rawWinRate <= 1 ? 100 : 1);
        const pf = report.profit_factor ?? getNumber(metrics, "profit_factor") ?? 0;
        const trades = report.trades_count ?? getNumber(metrics, "total_trades") ?? 0;
        const strategyName =
            strategy?.name ??
            report.dataset_name ??
            report.file_name ??
        ...
        console.error("[Certificate OG] Failed to generate image:", error);
