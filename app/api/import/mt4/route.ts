...
type TradeDirection = "long" | "short" | "unknown";
...
function parseMT4HtmlServer(html: string): ParsedTrade[] {
    ...
    for (let i = 0; i < Math.min(20, rows.length); i++) {
        const normalizedHeaders = rows[i].map((cell) =>
            cell.toLowerCase().replace(/[\s/]/g, "")
        );

        if (
            (normalizedHeaders.includes("type") ||
                normalizedHeaders.includes("ticket")) &&
            normalizedHeaders.includes("profit")
        ) {
            headerIdx = i;
    ...
    for (const row of rows.slice(headerIdx + 1)) {
        ...
        const typeRaw =
            typeIdx >= 0 ? row[typeIdx]?.toLowerCase() ?? "" : "";
        ...
        trades.push({
            ...
            direction: normalizeDirection(
                typeRaw.replace(/\s+stop|\s+limit/i, "")
            ),
            entry_price:
                openPriceIdx >= 0
                    ? parseNum(row[openPriceIdx]) || null
                    : null,
            exit_price:
                closePriceIdx >= 0
                    ? parseNum(row[closePriceIdx]) || null
                    : null,
            position_size:
                sizeIdx >= 0 ? parseNum(row[sizeIdx]) || null : null,
            open_time:
                openTimeIdx >= 0
                    ? parseDate(row[openTimeIdx] ?? "")
                    : null,
    ...
    try {
        ...
        if (trades.length === 0) {
            return NextResponse.json(
                {
                    error:
                        "No trades found in the MT4 statement. Make sure you exported the full account history as HTML.",
                },
                { status: 422 }
            );
        }
